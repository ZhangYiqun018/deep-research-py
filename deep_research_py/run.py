...
async def main(
    ...
    report_language_prompt = "🌐 Report language (en/zh) [zh]: "
    report_language = (await async_prompt(report_language_prompt)) or "zh"
    console.print()
    ...
    with Progress(
        ...
        report_dict = await write_final_report(
        ...
        console.print(Panel(report, title=report_title))
        ...
        with open(f"output/{report_title}.md", "w") as f:
            f.write(report)
        console.print(f"\n[dim]Report has been saved to output/{report_title}.md[/dim]")
